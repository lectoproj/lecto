...
export const POST = async (req: Request) => {
  ...
  if (!texto || !Array.isArray(questionAnswers) || questionAnswers.length === 0) {
    return NextResponse.json(
      { error: 'Missing texto or questionAnswers' },
      { status: 400 }
    );
  }

  const evaluations = await Promise.all(questionAnswers.map(async ({ tipo, pregunta, answer, correcta }) => {
    if (!pregunta || !answer) {
      return { tipo, pregunta, correcta, razon: 'Incomplete data provided', answer, isvalid: false };
    }

    // Generate the evaluation from the model based on the provided question, answer, and texto
    const { object } = await generateObject({
      model: model,
      system: 'Eres un especialista en evaluar respuestas de preguntas basadas en un texto proporcionado para niños de 10 años en primaria. Tu evaluación ser clara y concisa, y debe ser fácil de entender para los niños. Redacta hablando de manera singular, en segunda persona, y en tiempo presente.',
  ...
  await db.insert(lectura).values({
    ...
    nombre_lectura: 'El Robot Ayudante de Lucas',
    lecturas3: s3Url,
