...
export const GET = async (req: Request, res: Response) => {
    ...
    const { object } = await generateObject({
        model: model,
        system: 'Eres una IA especializada en crear preguntas de inferencia y su respuesta, estas tienen relevancia pedagógica para niños de primario de 10 años',
        prompt: 'Escribe una pregunta y su respuesta correcta',
        schema: z.object({
            objetoCloze: z.object({
              pregunta: z.string().describe("Pregunta de inferencia"),
              respuesta: z.string().describe("Respuesta"),
            }),
          }),
      });
