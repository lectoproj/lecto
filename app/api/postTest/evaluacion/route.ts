  ...
  try {
    ...
    if (!session?.user) {
      return NextResponse.json(
        { error: 'You must be logged in to generate' },
        { status: 401 }
      );
    }
    ...
    await db.transaction(async (trx) => {
      ...
      await trx.insert(evaluacion).values({
        ...
        retroalimentacion:
          parsedData.score?.gained > 6
            ? 'Felicitaciones! respondiste correctamente toda la evaluación. Buen trabajo, a continuar mejorando'
            : 'Esfuerzate un poco más la siguiente vez',
      ...
      const fillInTheBlankQuestions = parsedData.Quiz.filter(
        (item) => item.tipo === 'fill-in-the-blank'
      );
      ...
      const textQuestions = parsedData.Quiz.filter(
        (item) => item.tipo === 'text'
      );
      ...
      const mcqQuestions = parsedData.Quiz.filter(
        (item) => item.tipo === 'mcq'
      );
      for (const question of mcqQuestions) {
        const allTestObtencionInfo =
          await trx.query.testobtencioninfo.findMany();
